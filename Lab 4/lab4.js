...
function initGL(){
    ...
    numVertices = 2440;
    numTriangles = 4871;
    var vertices = getVertices(); // vertices and faces are defined in teapot20.js
    ...
    var eye = vec3( 50.0, 60.0, 70.0 ); // eye
    ...
    P_orth = [
        2.0/(rightPlane-leftPlane), 0.0, 0.0, 0.0,
        0.0, 2.0/(topPlane-bottomPlane), 0.0, 0.0,
        0.0, 0.0, -2.0/(farPlane-nearPlane), 0.0,
        -(rightPlane+leftPlane)/(rightPlane-leftPlane), -(topPlane+bottomPlane)/(topPlane-bottomPlane), -(farPlane+nearPlane)/(farPlane-nearPlane), 1.0
    ]

    P_persp = [
        2.0*nearPlane/(rightPlane-leftPlane), 0.0, 0.0, 0.0,
        0.0, 2.0*nearPlane/(topPlane-bottomPlane), 0.0, 0.0,
        (rightPlane+leftPlane)/(rightPlane-leftPlane), (topPlane+bottomPlane)/(topPlane-bottomPlane), -(farPlane+nearPlane)/(farPlane-nearPlane), -1.0,
    ...
    addPointLight = 1;
    addPointLightLocation = gl.getUniformLocation(myShaderProgram, "addPointLight");
    gl.uniform1f(addPointLightLocation, addPointLight);

    addDirectionalLight = 0;
    addDirectionalLightLocation = gl.getUniformLocation(myShaderProgram, "addDirectionalLight");
...
function getVertexNormals( vertices, indexList, faceNormals, numVertices, numTriangles ) {
    ...
    for (var j = 0; j < numVertices; j++) {
        ...
        for (var i = 0; i < numTriangles; i++) {var i0 = indexList[3*i];
